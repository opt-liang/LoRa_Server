...
#ifndef BINARY_SEARCH_VECTOR_HPP
#define BINARY_SEARCH_VECTOR_HPP
...
namespace BinarySearchVector
{
	...
	public:
		...
		void CancelAnyPendingDeletionRequest() {requestingDeletion = false;}
	...
	public:
		...
		List(uint32 myListTickPeriod_ms, uint32 myElementTickPeriod_ms, uint32 myElementTimeout_ms, bool myDeleteInternally) 	// myElementTickPeriod_ms must not be zero
			: nextTickedIndex(0),
			...
			else if (nextTickedIndex < Size())
			{
				...
				for (; tickCounter < end && nextTickedIndex < Size(); tickCounter++)
				{
					...
					if (deleteElement)
					{
						if (deleteInternally)
						{
							element = RemoveByIdPrivate(id);	//element is locked
							end--;	// don't increment index - all higher elements have moved down by 1

							if (!element && Debug::Print(Debug::major))
							{
							...
							return id;	//return id to be deleted
					}
					else
						nextTickedIndex++;
				}
