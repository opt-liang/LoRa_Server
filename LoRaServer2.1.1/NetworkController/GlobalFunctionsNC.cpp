...
{
	moteList.DeleteAllElements();	//active motes
}
...
void Global::ServerSpecificTick()
{
	...
	for (;;)
	{
		EuiReminderQueue::Reminder reminder = moteReminderQueue.Tick(now_ms);

		if (reminder.Eui() == invalidEui)
			break;

		Global::moteList.ReminderReceived(reminder.Eui(), reminder.Token());
	}
}
